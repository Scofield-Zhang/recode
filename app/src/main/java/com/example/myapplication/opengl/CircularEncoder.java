...
public class CircularEncoder {
    ...
    private static class EncoderThread extends Thread {
         ...
         * way through.
         * 我们将耗尽编码器以获得任何延迟的数据，但是我们不会关闭编码器或使用其他技巧来尝试“刷新”编码器。
         * 这可能意味着我们错过了最后几个提交的框架，如果他们仍然在通过。
         * <p>
         ...
         * 我们可能希望在此之后重置缓冲区 - 如果他们立即再次点击“捕获”，则最终将保存视频，并将其暂停写入文件。
         */
        void saveVideo(File outputFile) {
            ...
            if (index < 0) {
                Log.w("nihao", "Unable to get first index");
